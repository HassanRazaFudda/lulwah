...
- **Card payment is honestly gated, not faked**: `payment-intent {method:"card"}` cleanly `503`s in this environment (no live Ziina account — see §4.6.4, P3's Stripe→Ziina swap). Selecting card shows a real "temporarily unavailable, use Cash on Delivery" state rather than a payment form that could never actually charge anything here. **Not yet updated for the Ziina hosted-redirect flow** — this storefront checkout UI still reflects the pre-P3 shape (built when the gateway was still Stripe); wiring the actual redirect-out/redirect-back UX against Ziina's `redirectUrl` is P3's one remaining loose end, deliberately scoped out of the admin-focused P3 work — see §11.
...
- **Cart/checkout** (storefront): real since P2 (§5.4) — client-local state is gone. **Not yet updated for Ziina**: the checkout UI's payment step still reflects the pre-P3 (Stripe-era) shape; wiring the actual hosted-redirect UX against Ziina's `redirectUrl` is the one concrete piece of unfinished P3 work — see §11.
...
3. **Run `pnpm db:sync-indexes` as part of the real deploy pipeline** (§8.15, §25.2) — it doesn't currently appear there; without it, every unique index in the system silently stops being enforced on a fresh environment, exactly as it was for the entire build until this phase.
