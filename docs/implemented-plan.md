...
- ~~Dashboard's "orders needing action" and "low stock" tiles now read real data too; the four top summary stat tiles stay placeholder (no reports/analytics API module exists).~~ **Fixed** — see §6.12: the four stat tiles are real now too, backed by the `report` module's Sales endpoint.
...
**Settings**: a real form against the DB-backed values §4.7.5 describes — store details/TRN, the per-emirate shipping-rate table, free-shipping threshold, COD fee/cap, tax rate, feature-flag toggles, maintenance-mode toggle, and a read-only payment-gateway status card. The UI states plainly that `maintenanceMode`/feature flags persist for real but nothing in the codebase currently reads either field yet (verified by tracing every call site) — the toggles work, they just have no live effect. **Users & roles**: a real list against `GET /admin/users`, a real role-change control (typed confirmation + a client-side guard against self-role-change), and — closed in a later pass, see §11 item 8 below — three of the four remaining plan.md §11.1 actions are now real too: **invite staff** (`POST /admin/users/invite`, a generated one-time temporary password shown once to the inviting admin — no email infra exists to deliver it any other way), **deactivate/reactivate** (`PATCH /admin/users/:id/status`, toggling the pre-existing `User.status` field between `active`/`suspended`; suspending immediately revokes every active session for that user, not just blocking their next login, and login itself now checks `status` where it previously didn't at all), and **session list + revoke** (`GET/POST /admin/users/:id/sessions*`, a real read over the existing `Session`/refresh-token-rotation collection — one row per still-logged-in device, since a token rotates on every refresh; revoking one session doesn't touch the rest of the user's logins). **Force 2FA reset stays exactly as it was** — no real TOTP/2FA system exists anywhere in this codebase (confirmed again before leaving it), so there is nothing genuine to reset; it remains a visibly disabled "coming soon" control with its reason stated in the UI, deliberately not built against nothing.
...
### 6.12 Dashboard — the four top stat tiles, now real (`plan.md` §11.1, §11.2 rule 1)

Closes the one placeholder §6.4/§9 called out by name: `lib/placeholder-dashboard.ts`'s own doc comment said plainly "no `reports` API module exists yet... these four numbers stay placeholder." That module merged in P3 (§4.7.4) — this pass wires the tiles to it for real via a new `lib/queries/dashboard.ts#useDashboardSalesStats`, reusing `lib/queries/reports.ts#useSalesReportQuery` (already built for the Reports screen, §6.9) rather than duplicating its fetch/validation logic. `lib/queries.ts` and `lib/placeholder-dashboard.ts` are both deleted — confirmed nothing else referenced either first.

Verified against `report.dto.ts`/`report.ts` (`@lulwah/contracts`)/`sales.repository.ts` before writing any of this, not assumed:

- **Revenue (7d)** = `SalesReportResponse.totals.grandTotalFils` for a trailing 7-day window (two real `GET /admin/reports/sales` calls, full ISO timestamps rather than the Reports screen's date-only strings, to avoid the UTC-midnight truncation a date-only `dateTo` would cause on "today").
- **Orders (7d)** = `totals.ordersCount`, same response.
- **Avg. order value** = `grandTotalFils / ordersCount`, derived client-side — no endpoint computes AOV directly, but both inputs are real numbers from the same real response, so this is an honest derivation, not a fabrication.
- **"Conversion rate" was dropped, not faked, and replaced with Units sold (7d)** (`totals.unitsSold`). No GA4/analytics integration exists anywhere in this repo (§4.7.4, §11 item 10) — there is no visit/session count to divide orders by, so there is no real number to compute. Units sold is the fifth metric plan.md's own §11.1 sentence names in the same breath ("revenue, orders, AOV, conversion rate, units"), and one the same Sales response already computes for real — the honest substitute rather than a silently-fake fourth tile next to three real ones.
- **Sparkline trend vs previous period**: built for real, not faked — `SalesReportQuery` genuinely accepts arbitrary `dateFrom`/`dateTo`, so a second call fetches the 7 days immediately before the current window and a real percent-change is computed from the two real totals. No sparkline *chart* widget exists in this app (`StatTile`'s own doc comment already noted this — the signed delta is the existing convention this reuses, not a new gap).

Every admin role that can reach this screen already has `reports.read` (`identity.policy.ts`'s `ROLE_PERMISSIONS` — `super_admin`/`manager`/`catalog`/`order_ops`/`content`/`finance` all list it), so a 403 here is effectively unreachable in practice; a defensive `isForbiddenError` branch (reusing `AccessDenied`, same as Reports/Audit log) is wired anyway rather than assumed away.

Live-verified against the real Docker dev stack: logged in as the seeded `super_admin` and called `GET /admin/reports/sales` directly with the exact same date-window logic this hook computes (current 7d + prior 7d) — got back 5 real orders / 8 units / AED 1,853.00 revenue for the current window (all 5 seed-era orders fall inside it) and a genuine all-zero previous window, confirming the tiles' underlying numbers are real and the code's date-math matches what the API actually returns. Also confirmed `pnpm --filter @lulwah/admin typecheck/lint/test/build` all pass clean (74/74 tests) and that the real `next dev` server serves the dashboard route with no server-side error. **Not verified**: an actual browser render of the four tiles with a logged-in session — no browser-automation tool is available in this environment, same limitation this document's every other admin-screen live-verification note already states.
...
7. ~~**The full §10.2 RBAC permission matrix**~~ **Done** — every cell of the eleven-column/nine-role table in `identity.policy.ts#ROLE_PERMISSIONS` was cross-checked against `plan.md` §10.2 by hand, one real mismatch found and fixed (`manager` was missing `reports.write` despite the table's ✏️ on Reports for that role — every other role/column already matched). The harder half of the same footnote — `warehouse`/`support`'s `✏️*` restricted-transition-subset — was previously undocumented-but-unenforced: `order.service.ts#updateOrderStatus` granted either role the *entire* transition table the moment they held `orders.status.update`, identical to `manager`/`order_ops`. Fixed with a new `order.transitions.ts#assertRoleMayMakeTransition`, called from `updateOrderStatus` on top of (never instead of) the existing table check — `warehouse` is now hard-restricted to `processing→ready_to_ship`/`ready_to_ship→shipped`, `support` to any table-valid transition whose target is `cancelled`, and every other role (`super_admin`'s force escape hatch included) is untouched. "Support may also re-send notifications" (the other half of the footnote) has no corresponding endpoint anywhere in this codebase — there is no standalone "resend" action, only the `notifyCustomer` flag on a status change itself — so that half stays honestly unenforced/aspirational rather than invented against nothing. Exhaustive test coverage added: 288 new `(from, to)` cases across both restricted roles in `order.transitions.test.ts`, plus two full HTTP-level integration tests in `checkout.integration.test.ts` proving a `warehouse` actor is rejected (403) on a transition that's genuinely valid in the table for other roles but outside their own subset.
8. ~~**Users & roles' missing endpoints**~~ **Three of four done** — see §6.10. Invite staff, deactivate/reactivate, and session-list-with-revoke are now real endpoints in `identity.routes.ts`, wired to the previously-disabled admin buttons. **Force 2FA reset is the one deliberately left as-is** — no real TOTP/2FA system exists anywhere in this codebase to reset (confirmed again, not assumed), so it stays a visibly disabled control rather than a button wired to nothing real.
9. ~~**Dashboard summary stat tiles** (§6.4, §9) — still placeholder; `report`'s new per-category endpoints (§4.7.4) could back these with a small aggregation, now that they exist.~~ **Done** — see §6.12. Revenue/Orders/AOV/Units sold (7d, vs the prior 7d) are wired to the real Sales report; "Conversion rate" was deliberately dropped rather than faked, since no analytics/GA4 integration exists anywhere in this repo to supply a real visit count (item 10 below, unchanged).
