...
**Mega menu** — full-width panel. Four columns of links + a fifth **featured tile** (campaign image + collection name) that crossfades as you hover a category. ~~Brands get their own column with wordmarks.~~ Superseded — see `docs/adr/0001-remove-brand-listing.md`: Lulwah Fashion sells its own product, not a multi-brand assortment, so no brand column. Sale link in garnet.
...
5. ~~**Brand strip** — Khaadi, Asim Jofa, Sana Safinaz and others as a slow marquee of wordmarks; each links to a brand house page.~~ Superseded — see `docs/adr/0001-remove-brand-listing.md`: Lulwah Fashion sells its own product, not a multi-brand assortment, so this slot no longer exists in the launch order.
